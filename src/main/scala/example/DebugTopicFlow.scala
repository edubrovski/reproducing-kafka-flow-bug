...
object DebugTopicFlow {
  ...
  def of[F[_]: Async: Concurrent: Parallel: LogOf](
    consumer: Consumer[F],
    topic: Topic,
  ...
  private def of[F[_]: Async: Concurrent: Parallel: Log](
    consumer: Consumer[F],
    topic: Topic,
    partitionFlowOf: PartitionFlowOf[F],
      ...
      def apply(records: ConsRecords) = {
        ...
        for {
          ...
          _ <- partitions.toList parTraverse { case (partition, flow) =>
            ...
            for {
              _ <- flow(partitionRecords).onCancel(Concurrent[F].raiseError(new Exception("PartitionFlow.apply canceled")))
            } yield ()
          }
